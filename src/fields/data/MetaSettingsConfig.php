...
class MetaSettingsConfig
{
    ...
    {
        return $this->normalizeOptions($this->_options);
    }
    ...
    public function options(): array
    {
        ...
        return collect($this->_options)
            ...
            ->filter(fn($item) => !isset($item['optgroup']))
            ->all();
    ...
    private function load( string $source, $context = [] ): void
    {
        ...
        if (is_array($json)) {
            ...
            $this->setError('Unable to Parse Config', [
    ...
    public function verify(array $data = []): array
    {
        ...
        // Ensure constants are always set
        ...
        foreach ($schema['settings'] ?? [] as $field) {
            ...
            if (in_array($type, ['text','textarea','email','url','icon','money','date','time'])) {
                $clean['json'][$name] = $data['json'][$name]
                    ?? $field['default']
                    ?? $field[$type]['default']
                    ?? '';
            }

            // normalize options for select-like types
            ...
            // enum types
            if (in_array($type, ['select','radiogroup'])) {
                $selected = $data['json'][$name] ?? $field['default'] ?? '';
                $matched  = collect($field['options'])->firstWhere('value', $selected) ?? null;

                if (!$matched) {
                    // choose first non-optgroup option as fallback
                    $matched = collect($field['options'])->first(fn($o) => empty($o['optgroup']));
                    $clean['valid'] = false;
                }
            ...
            // lightswitch
            if ($type === 'lightswitch') {
                $state = $data['json'][$name]
                    ?? $field['default']
                    ?? $field['lightswitch']['default']
                    ?? false;
        ...
        // keep json as array; let Field::serializeValue() encode later
    ...
    /**
     * Normalize a field's option list to Craft's flat optgroup format.
     */
    private function normalizeOptions(array $options): array
    {
        ...
        foreach ($options as $item) {
            // Nested optgroup? Flatten it
            if (isset($item['options']) && is_array($item['options'])) {
                $label = $item['optgroup'] ?? $item['label'] ?? 'Group';
                $normalized[] = ['optgroup' => $label];

                foreach ($item['options'] as $sub) {
                    $normalized[] = [
            ...
            // Existing optgroup marker
            if (!empty($item['optgroup'])) {
                $normalized[] = ['optgroup' => $item['label'] ?? $item['optgroup']];
                continue;
            }
