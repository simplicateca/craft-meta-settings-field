...
class MetaSettingsField extends BaseOptionsField implements SortableFieldInterface, InlineEditableFieldInterface
{
    ...
    private static array $_templateCache = [];
    ...
    {
        $config = new MetaSettingsConfig( $this->fieldConfig(), $element, [ "handle" => $this->handle ] );
    ...
    public static function listConfigTemplates(): array
    {
        $path = Craft::$app->getPath()->getSiteTemplatesPath();
        if (!isset(self::$_templateCache[$path])) {
            self::$_templateCache[$path] = FileHelper::findFiles($path, [
                'only' => ['*.json', '*.twig'],
                'recursive' => false,
            ]);
        }

        return self::$_templateCache[$path];
    }
}
